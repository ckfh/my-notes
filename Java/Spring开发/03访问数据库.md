...
> JdbcTemplate比起原生JDBC，减少了手动编写try...finally代码块的工作量，但是对于其它工作，两者之间代码量相差无几。当然JdbcTemplate也提供了许多便捷的方法快速实现JDBC操作。
...
Spring提供的JdbcTemplate采用Template模式，提供了一系列以**回调**为特点的工具方法，**目的是避免繁琐的try...catch语句。**注意以下大部分的JdbcTemplate方法都包含有一个**函数式**接口参数，我们通过lambda表达式创建子类，并编写该接口唯一方法的代码逻辑从而实现JDBC操作。
...
public User getUserById(long id) {
    ...
    return this.jdbcTemplate.execute((Connection conn) -> {
        // 我们不用担心参数从何注入，有了Connection我们可以做任何基于Connection的操作：
        // 直接使用Connection实例，也不需要释放它，回调结束后JdbcTemplate自动释放：
        // 但是对于之后获取的PreparedStatement和ResultSet仍然需要使用try(...)代码块进行释放：
        // 此类方式已接近于原生的JDBC操作：
...
public User getUserByName(String name) {
    ...
    return this.jdbcTemplate.execute("SELECT * FROM users WHERE name = ?", (PreparedStatement ps) -> {
        // 直接使用JdbcTemplate帮我们创建好的PreparedStatement搭配SQL语句，同样在回调结束后自动释放：
        ps.setObject(1, name);
        try (ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return new User(rs.getLong("id"), rs.getString("email"), rs.getString("password"), rs.getString("name"));
...
public User getUserByEmail(String email) {
    ...
    return this.jdbcTemplate.queryForObject("SELECT * FROM users WHERE email = ?", new Object[]{email}, (ResultSet rs, int rowNum) -> {
        // 传入SQL、参数、以及RowMapper实例，直接使用ResultSet，rowNum用于接收当前行的行号：
...
public List<User> getUsers(int pageIndex) {
    ...
    // query()方法针对的是多行记录，直接实例化一个RowMapper的子类BeanPropertyRowMapper，并传入一个JavaBean的Class：
    // 如果表结构和JavaBean属性名称一致，那么BeanPropertyRowMapper可以直接把一行记录按列名转换为JavaBean：
...
public void updateUser(User user) {
    ...
    // 使用update()方法进行插入、更新和删除操作，传入SQL语句及位置参数，返回更新的行数
...
public User register(String email, String password, String name) {
    // 如果某一列是自增列（例如自增主键），通常，我们需要获取插入后的自增值。
...
编写数据访问层的时候，可以使用DAO模式。DAO即Data Access Object的缩写。即当我们需要插入user表的一行时，我们不再传入组成一个User对象的所有属性参数，而是选择传入一整个User对象，尽可能以**对象**为单位进行各项数据库操作。
...
// 实际上就相当于从多个子类中将注入JdbcTemplate的操作给抽取到了父类当中。
@Component
@Transactional
