...
> JdbcTemplate只是对原生JDBC的一个轻微封装，目的是为了减少手动编写try...finally代码块的工作量，但仍依旧需要手动编写SQL语句。
...
public List<User> getUsers(int pageIndex) {
    ...
    // 如果表结构和JavaBean属性名称一致，那么jdbcTemplate可以根据BeanPropertyRowMapper可以直接把一行记录按列名转换为JavaBean：
