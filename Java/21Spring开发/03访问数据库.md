...
针对每一个JavaBean，例如user类、goods类，我们应当为它们各自建立相应的XxxDao类**封装**它们各自相关的数据持久化操作。或者说数据库当中已有一个日期表，那我们应当先建立一个日期类，然后建立对应的dao类，其各种成员方法都是针对日期类的属性到日期表的持久化操作。

这样后续编写业务层代码时，只需编写业务逻辑，具体的数据操作则通过调用数据访问层当中封装好的各式方法进行操作，自身不会出现任何有关对数据库操作的代码。
...
注意findByCriteria()方法可以提供额外的两个参数来限定返回的数据个数，通常用在分页查询当中。
...
HibernateTemplate的load()方法源码，本质上就是使用session来完成JDBC操作：
