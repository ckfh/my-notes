        ...
        if (root != null) {
            ...
            while (!stack.isEmpty() || root != null) {
                if (root != null) {
                    ...
                    // 走到底，弹出栈顶元素进行访问，切换到右孩子：
                    root = stack.pop();
                    // 注意向左走的时候是先经过根节点再到左孩子的，左孩子被访问后就是访问的根节点：
                    list.add(root.val);
                    root = root.right;
                }
            }
...
使用已经建立好的当前层next指针在当前层移动并构造下一层的next指针。
...
层序遍历的思路同样适用于该题，上一题是完美二叉树，该题是普通二叉树。
...
该题递归思路：以后序遍历为模板，先访问左右节点再处理当前节点（**递归中你用来处理当前节点的方式同样也是用来处理左右节点的方式**）；自底向上处理到根节点；
...
public class Codec {
    public String serialize(TreeNode root) {
        ...
        // 递归中如何处理当前节点就是如何处理左右节点
        String res = root.val + "!";
        res += serialize(root.left);
        res += serialize(root.right);
        // 上一层递归需要这一层递归的值，所以需要将值返回
        return res;
    }
...
对节点进行函数调用时，实际上就代表着对节点进行了访问，每访问一个节点就将其右孩子设为被访问的左孩子，左孩子设为被访问的右孩子。
...
class Solution {
    public TreeNode mirrorTree(TreeNode root) {
        ...
        // 因为右指针已经指向左孩子，因此需要先暂存右孩子节点，然后用左指针指向右孩子：
        root.left = mirrorTree(tmp);
        // 你如何处理当前节点就是如何处理孩子节点，这里我们将节点直接返回：
        return root;
    }
}
```
