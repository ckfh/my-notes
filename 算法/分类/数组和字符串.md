...
class Solution {
    ...
    public String reverseWords(String s) {
        // 注意点一：如果这里用正则`\\s+`，则循环中的第一个判断条件不需要，但是指定字符的匹配比起正则表达式的匹配更快
        String[] words = s.trim().split(" ");
        // 注意点二：如果这里用`StringJoiner`，则循环中的第二个判断条件不需要
...
从两端向中间迭代数组。技巧：一个指针从头部开始，而另一个指针从尾部开始。
...
```java
class Solution {
    // 数组有序，那就从两边起向里靠拢，值大了就把右边界往里一点，值小了就把左边界往里一点
...
涉及到连续子数组，通常有两种解法，一种是滑动窗口，一种是前缀和。
