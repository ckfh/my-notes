    ...
    public String decodeString(String s) {
        ...
        for (char code : codes) {
            if (code != ']') {
                ...
                stack.pop();
                // 这里记录子字符串的迭代次数:
                StringBuilder loopCount = new StringBuilder();
                while (!stack.isEmpty() && (stack.peek() >= '0' && stack.peek() <= '9')) {
                    loopCount.append(stack.pop());
                }
                int n = Integer.parseInt(loopCount.reverse().toString());
                // 这里将子字符串按照迭代次数放入栈中:
                char[] loopCodes = sb.reverse().toString().toCharArray();
                for (int i = 0; i < n; i++) {
                    for (char loopCode : loopCodes) {
                        stack.push(loopCode);
                    }
                }
            }
...
class Solution {
    ...
    private boolean isOK(int sr, int sc) {
        return sr >= 0 && sr < this.width && sc >= 0 && sc < this.length && this.image[sr][sc] == oldColor;
    }
...
或使用显式栈实现DFS。使用while循环和栈来模拟递归期间的系统调用栈。
...
class Solution {
    ...
    private void helper(int roomNum) {
        if (this.used.contains(roomNum)) {
            return;
        }
        this.used.add(roomNum);
        for (Integer keyNum : this.rooms.get(roomNum)) {
            helper(keyNum);
        }
    }
}
